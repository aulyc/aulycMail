# aulycmail Extension System

Developer reference for building first-party extensions on top of aulycmail's core.
...
This doc is the contract every aulycmail extension uses to interact with the host and with other extensions. Every claim is backed by a file path you can read directly — no second source of truth.
...
```
┌─────────────────────────────────────────────────────────────────────┐
│  aulycmail process (single binary, single WebKit view)                 │
│                                                                     │
│  ┌────────────────────────┐    ┌──────────────────────────────┐    │
...
Package: `github.com/aulyc/aulycmail/internal/core/api/v1` (12 files; entirely interface + type declarations, no logic).
...
aulycmail is still pre-1.0 — the surface may continue to evolve as more first-party extensions surface their needs.
...
- **macOS:** delegates to the system `open` handler.
...
- `MessageRef{ AccountID, FolderID, MessageID }` — aulycmail DB id, not RFC 5322 Message-ID
...
aulycmail's testing style is integration-flavored: a real SQLite at `t.TempDir()` is fast enough (~10ms per open) and exercises actual SQL behavior. Avoid mock layers unless the dependency is genuinely external (an HTTP server — use `httptest.Server`).
...
Two steps to add an extension-pulled npm dep:
...
This pattern is anchored in [the lightweight-by-default motto](../README.md) — aulycmail remains a simple email client for users who don't enable extensions, and extensions opt-in to features at the cost of weight. Mail must never carry kit overhead.
...
The kit reuses aulycmail's existing pane-focus store at [`frontend/src/lib/stores/keyboard.svelte.ts`](../frontend/src/lib/stores/keyboard.svelte.ts). The slot type is `'sidebar' | 'messageList' | 'viewer'` — those names are kept as-is for backward compatibility with mail's existing focus dispatch. Extension panes register against these same slots:
...
7. **Verify the lightweight invariant**: with the new component built but no extension enabled, htop should show no aulycmail/webkit2gtk activity. The kit must be lazily mounted only when an extension is active.
...
**1. Consume `coreapi` only. Never reach into aulycmail internals.**
...
The Contacts extension *is allowed* to receive a `*database.DB` handle to aulycmail's main DB because it's a first-party special case: it grew out of code that already lived in aulycmail core, owns the lifecycle of the `contact_records` + `contact_sources` + related tables, and is explicitly the canonical owner of that data. **This special case is not extended to new extensions.** A new extension never gets a handle to `aulycmail.db`.
...
That's it. **No other host file should change.** If you find yourself editing files outside the points above and `extensions/<name>/`, you're either (a) reaching into aulycmail internals (forbidden — see hard rule #1), or (b) trying to add a `coreapi` method (forbidden — file a Feature Request first).
...
All extensions compile into the single aulycmail binary. Extensions live as Go packages under `extensions/<name>/backend/` and Svelte components under `extensions/<name>/frontend/`. The host embeds each extension's `*Bridge` on App (one wiring file at `app/extension_<name>.go`, ~28 LOC) and iterates `a.knownExtensions` to call `Register()` at startup.
...
If third-party-PR intake opens and demand grows past what individual code reviews can sustain, the next step the architecture allows is a **pre-compiled subprocess + IPC** model for community extensions. Each community extension would ship as its own Go binary (cross-compiled per platform), launched as a subprocess at startup, communicating with the main app via Unix socket / named pipe — the same path aulycmail already uses for the detached composer.
