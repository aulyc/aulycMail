...
This guide walks you through rolling back aulycmail's database schema after an upgrade. Schema migrations in aulycmail are **forward-only by design** — the running application doesn't downgrade automatically. If you upgrade aulycmail to a new version that ran a migration and then want to go back to an older version, you need to manually reshape the database so the older version can read it.
...
- You upgraded to a newer aulycmail (e.g., 0.3.0) and want to go back to 0.2.5 for any reason.
- aulycmail shows an error like *"This database was written by a newer aulycmail. Either upgrade aulycmail or follow the rollback guide..."* — the schema-version gate is refusing to open a DB that's newer than your running build.
...
- The matching rollback script for your migration, downloaded from the aulycmail repo's `tools/db/` directory on the branch/tag corresponding to the version that introduced the migration.

---

### Procedure

1. **Quit aulycmail completely** (use the menu Quit, or kill the process — make sure nothing is using `aulycmail.db`).
...
2. **Back up your DB file** as a precaution. This script makes changes that are difficult to reverse cleanly if anything goes wrong, so a real file copy is your safety net:
   ...
   ```bash
   cp ~/.local/share/aulycmail/aulycmail.db ~/.local/share/aulycmail/aulycmail.db.before-rollback
   ```

   (Adjust the path for your OS — see "What you need" above.)

3. **Download the rollback script** from the aulycmail repo. On the branch where the 0.3.0 schema lives (0.3.0 or later):

   ```bash
   curl -O https://raw.githubusercontent.com/aulyc/aulycmail/main/tools/db/rollback-v39-to-v30.sql
   ```

   (Or download via your browser from `https://github.com/aulyc/aulycmail/blob/main/tools/db/rollback-v39-to-v30.sql`.)
...
5. **Verify the rollback** worked:
   ...
   ```bash
   sqlite3 ~/.local/share/aulycmail/aulycmail.db \
...
6. **Launch the older aulycmail** (0.2.5 or earlier). It should start normally and your contacts autocomplete should work as before.
...
```bash
cp ~/.local/share/aulycmail/aulycmail.db.before-rollback ~/.local/share/aulycmail/aulycmail.db
```

You're back to the v39 state and can run aulycmail 0.3.0 again.

If the issue persists, open a GitHub issue with the SQL error output and the version you were rolling back from / to.

---

## Rollback: v39 → v30 (aulycmail 0.3.0 → 0.2.5)

**Introduced in**: aulycmail 0.3.0 (cumulative effect of migrations 31 + 32 + 33 + 34 + 35 + 36 + 37 + 38 + 39 — see notes below).
...
- The `vcard_raw` round-trip preservation column. Means that the next time CardDAV sync runs under the older aulycmail, it will re-fetch and re-parse vCards from the server — only the fields the older parser knows about (email + display name) survive.
- CardDAV contacts' synthetic local IDs are reshaped. Older aulycmail identifies CardDAV contacts during sync by `href` (server-side URL path), not by local ID, so this doesn't affect sync correctness — only the row IDs change.
- Local-record UUIDs are dropped — v30 keys local contacts by email, which is the natural identity for the legacy schema.
- **Historical extension secrets stored in the AES-fallback path** (i.e., entries in the `extension_secrets` table). Keyring-stored entries are NOT touched by the rollback SQL — they remain in the OS keyring but become orphaned (no DB row pointing at them). To clean them up, use your OS keyring manager (Seahorse / Keychain / Credential Manager) and remove entries starting with `ext:`.
- **Legacy OAuth token rows and fallback columns**, if your database came from a pre-removal build that used them. The rollback deletes non-mail `oauth_tokens` rows and drops the v36 fallback columns. Current aulycmail builds no longer read OAuth tokens. Per-slot keyring entries (keys of the form `<accountID>:<configID>:access_token` / `refresh_token`) are NOT cleared by the rollback SQL; remove them from the OS keyring manager if you want a clean state.
