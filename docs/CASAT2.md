...
This document provides detailed responses to the Cloud Application Security Assessment (CASA) Tier 2 Self-Assessment Questionnaire for [aulycmail](https://github.com/aulyc/aulycmail), a lightweight cross-platform email client.
...
aulycmail does not use any unsupported, insecure, or deprecated client-side technologies. Specifically, there is no use of:
...
aulycmail is a local desktop application. All access controls are enforced in the **Go backend** — never in the frontend (Svelte/WebView). The frontend acts solely as a presentation layer and has no direct access to external services, local files, or credential storage.
...
All sensitive data in aulycmail is identified and classified into the following protection levels:
...
- **No telemetry or analytics**: aulycmail collects no usage data and makes no network requests beyond user-configured mail/contact/key servers and OAuth providers.
...
aulycmail does not load or execute code from untrusted or external sources at runtime:
...
aulycmail is a **local desktop application** with no web infrastructure. It does not operate or depend on:
...
The only external DNS dependency is:

- **github.com/aulyc/aulycmail** — Source code repository and release distribution (actively maintained by the project owner)

This does not involve project-owned DNS records or subdomains that could be subject to takeover. The application itself connects only to user-configured mail servers, OAuth providers (Google/Microsoft), and optionally CardDAV/key servers — all specified by the end user, not hardcoded infrastructure.

---

## 8. Anti-Automation Controls

aulycmail is a **local desktop application** that does not expose any network-accessible APIs, endpoints, or services. There is no attack surface for remote automation, mass data exfiltration, or denial-of-service attacks against the application itself.

The following built-in controls prevent excessive or abusive local operations:

- **Single-instance enforcement**: Only one instance of aulycmail can run at a time (Unix socket lock on Linux/macOS, named mutex on Windows), preventing parallel automated access to the database.
...
aulycmail is a desktop application with no web server or web root. There is no directory served over HTTP that untrusted files could be placed into.
...
- **Data directories**: All application data directories are created with 0700 permissions (owner-only access):
  - Linux: `~/.local/share/aulycmail/` (XDG Base Directory spec)
  - macOS: `~/Library/Application Support/aulycmail/`
  - Windows: `%LOCALAPPDATA%\aulycmail\`

---

## 10. Malicious Content Scanning

aulycmail is a desktop email client that does not upload or serve files to other users. There is no file hosting, sharing, or serving component.
...
aulycmail does not expose any API endpoints of its own. Where it consumes external APIs, no sensitive information is included in URLs.
...
aulycmail is a local desktop application with no HTTP server, URI routing, or multi-user access model. There are no URIs, controllers, or routers to enforce authorization against. However, authorization decisions are enforced at every access boundary:

**Process-level authorization:**

- **Single-instance lock**: Only one aulycmail process can access the database at a time (Unix socket lock on Linux/macOS, named mutex on Windows).
...
aulycmail does not expose any RESTful HTTP APIs, endpoints, or web services. There are no HTTP routes, controllers, or method handlers to restrict.
...
aulycmail uses a **Makefile** with well-defined, repeatable build targets:
...
- **Source builds**: Built from source via the documented `make build` target, ensuring the resulting binary matches the local source tree
...
aulycmail can be fully rebuilt from source at any time using documented, automated build commands:
...
aulycmail is a local desktop application with no server-side components, cloud infrastructure, or databases to restore. All user data (email, contacts, settings, credentials) is stored locally on the user's machine. In the event of data loss:
...
aulycmail is a single-user desktop application. The user is the sole administrator and has full control over all configuration.
...
- **Single-instance lock**: Only one aulycmail process can access the database at a time, preventing concurrent modification by unauthorized processes.
...
- **Schema migrations**: The database schema is versioned. aulycmail validates the schema version on startup and applies migrations sequentially — an unexpected schema state would indicate tampering.
...
Debug mode is **disabled by default** in aulycmail. It must be explicitly opted into by the user via:

- `--debug` command-line flag
- `AULYCMAIL_DEBUG=1` environment variable

Neither of these is set in production builds.
...
- **Production** (`make build`): Built without dev server or DevTools. The Wails WebView does not expose developer consoles or debug endpoints in production builds.
...
aulycmail does not expose any HTTP debug endpoints, status pages, profiling routes, or developer consoles. There is no web server — the application is a local desktop binary with an embedded WebView.

---

## 18. Origin Header Not Used for Access Control

aulycmail does not use the `Origin` header (or any HTTP header) for authentication or access control decisions.

- **No HTTP server**: aulycmail does not run a web server or expose HTTP endpoints. There are no incoming HTTP requests to inspect headers on.
...
aulycmail does not use cookies or cookie-based sessions. There is no HTTP server, no session management layer, and no cookies to protect.
...
- **CSRF not applicable**: Cross-site request forgery requires a web browser making cross-origin requests with attached cookies. aulycmail's WebView renders only embedded application assets and does not navigate to external sites. There is no attack vector for CSRF.

---

## 20. LDAP Injection

aulycmail does not use LDAP in any capacity. There are no LDAP queries, connections, directory binds, or search filters anywhere in the codebase.
...
- **No directory services**: aulycmail has no integration with Active Directory, OpenLDAP, or any other LDAP-based directory.
...
aulycmail is protected against both Local File Inclusion (LFI) and Remote File Inclusion (RFI) attacks.
...
aulycmail stores all data locally on the user's machine. There is no cloud backend, no server-side storage, and no third-party data sharing. Regulated private data is encrypted at rest as follows:
...
- **No telemetry or analytics**: aulycmail collects no usage data whatsoever.
...
- **Privacy Policy**: Published at [`docs/PRIVACY.md`](https://github.com/aulyc/aulycmail/blob/main/docs/PRIVACY.md).

---

## 23. Constant-Time Cryptographic Operations

aulycmail does not implement any custom cryptographic primitives or algorithms. All cryptographic operations are delegated to established, audited libraries that implement constant-time operations internally:
...
Application-level security comparisons (e.g., IPC token validation) use `crypto/subtle.ConstantTimeCompare`. All other cryptographic operations are handled by the libraries listed above. There are no short-circuit comparisons on secrets, tokens, or cryptographic material in the aulycmail codebase.
