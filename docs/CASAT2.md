...
- **Backend**: Go application exposing methods to the frontend via Wails bindings. Manages all business logic, synchronization, TLS handling, credential encryption fallback, and external service communication.
- **SQLite Database**: Local storage for messages, accounts, folders, contacts, drafts, settings, and trusted TLS certificate decisions. WAL mode for concurrent reads. File permissions restricted to owner (0600).
- **OS Keyring**: Primary credential storage for account passwords.
...
4. **HTML Rendering**: Raw email HTML → bluemonday sanitization (strip scripts, dangerous tags, tracking pixels) → WebView display
5. **Detached Composer**: Composer process ↔ token-authenticated IPC ↔ main process (for sending, draft save/delete)
...
- **Authentication**: IMAP/SMTP password credential handling is performed entirely by the Go backend. The frontend never sees raw credentials.
- **Credential Storage**: Passwords are stored via the OS keyring or an encrypted database fallback (AES-256-GCM). Access is managed entirely by the backend.
...
- **SQLite database** (0600 file permissions, owner-only): contains email messages, headers, contacts, drafts, account configurations, and trusted TLS certificate decisions
...
| **High** | Credential fields use AES-256-GCM fallback when keyring is unavailable; other local app data relies on OS/file protections | SQLite WAL mode with checksums | File permissions: DB 0600, directories 0700, IPC socket 0600 | User-controlled (all data is local, deletable) | No telemetry, no cloud sync, no third-party data sharing |
...
- **No telemetry or analytics**: aulycmail collects no usage data and makes no network requests beyond user-configured mail servers.
...
This does not involve project-owned DNS records or subdomains that could be subject to takeover. The application itself connects only to user-configured mail servers — all specified by the end user, not hardcoded infrastructure.
...
**External network I/O**: The only external connections are IMAP/SMTP over TLS — binary protocols, no HTTP URLs involved.
...
- **OS keyring**: Credentials and private keys are protected by the operating system's keyring access controls (user session scope).
...
- **IMAP/SMTP**: Authenticated per-account using password credentials stored in the keyring.
...
**External API consumption**: The Go backend consumes no external REST APIs; the only external network I/O is IMAP/SMTP over TLS. The frontend has no ability to make direct HTTP requests to external services.
...
- **Credentials**: Re-entered by the user (passwords)
- **Settings and state**: Regenerated with defaults on first launch
- **Contacts**: Collected contacts re-accumulate from sent mail; manually added contacts are restored from backups if available
...
Even when debug mode is explicitly enabled, log output does not contain sensitive information such as passwords, private keys, or email body content. Debug logging is limited to operational information (sync progress, connection states, error messages).
...
- **Authentication**: Handled via IMAP/SMTP password credentials — no LDAP directory authentication.
- **Contact lookup**: Local SQLite queries only — no LDAP or remote directory protocols.
...
- **No third-party data sharing**: No data is transmitted to any party other than the user's configured mail servers.
