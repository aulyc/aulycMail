...
- **Backend**: Go application exposing methods to the frontend via Wails bindings. Manages all business logic, synchronization, TLS/OAuth handling, credential encryption fallback, and external service communication.
...
- **OS Keyring**: Primary credential storage for passwords and OAuth tokens.
...
4. **OAuth2 Flow for Contacts/Extensions**: User initiates → local callback server on random port → authorization code with PKCE → token exchange over HTTPS → encrypted token storage
...
**Optional (user-enabled per account or extension):**

7. **CardDAV Contact Sync**: CardDAV server → HTTPS → Go backend → SQLite. Only active when user configures a CardDAV source.
...
- **Authentication**: IMAP/SMTP password credential handling, OAuth2 flows for contacts/extensions (with PKCE and state validation), and token exchange are all performed by the Go backend. The frontend never sees raw credentials or tokens.
- **Credential Storage**: Passwords and OAuth tokens are stored via the OS keyring or an encrypted database fallback (AES-256-GCM). Access is managed entirely by the backend.
...
- **No telemetry or analytics**: aulycmail collects no usage data and makes no network requests beyond user-configured mail/contact servers and OAuth providers.
...
This does not involve project-owned DNS records or subdomains that could be subject to takeover. The application itself connects only to user-configured mail servers, OAuth providers (Google/Microsoft), and optionally CardDAV servers — all specified by the end user, not hardcoded infrastructure.
...
- **OAuth client secrets**: User-supplied for contacts/extensions where configured; never hardcoded in source code
- **`.env` / `.env.local`**: Optional local development files, listed in `.gitignore`
...
- **Credentials**: Re-entered by the user (passwords) or re-authorized for configured contact/extension OAuth flows
...
- **External API calls**: Outbound requests to Google, Microsoft, and CardDAV authenticate via `Authorization` headers with bearer tokens or credentials — Origin is neither sent nor relied upon.
...
- **No third-party data sharing**: No data is transmitted to any party other than the user's configured mail/contact servers and OAuth providers.
