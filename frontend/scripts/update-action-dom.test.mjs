...
test('shares latest/update state, manually checks, and confirms installation', async () => {
  ...
  const updateButton = compact.querySelector('button')
  ...
  assert.equal(updateTitle.closest('button'), null)
  assert.ok(updateButton.classList.contains('border-input'))
  assert.ok(updateButton.classList.contains('bg-background'))
  assert.ok(updateButton.classList.contains('shadow-sm'))
