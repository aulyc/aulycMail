...
test('about tab opens the website and renders every information section', async () => {
  ...
  const metadata = document.querySelector('[data-about-section="metadata"]')
  assert.ok(metadata.classList.contains('grid-cols-[max-content_minmax(0,1fr)]'))
  assert.match(metadata.textContent, /0\.6\.0-test · build 42/)
  assert.match(metadata.textContent, /settingsAbout\.compatibilityValue/)
  assert.match(metadata.textContent, /settingsAbout\.systemRequirementValue/)
