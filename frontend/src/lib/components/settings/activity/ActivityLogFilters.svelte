...
<div class="flex flex-wrap items-center justify-between gap-3">
  ...
  <div class="flex items-center gap-2">
    <label class="flex items-center gap-2 text-xs text-muted-foreground">
      ...
      <input type="date" value={store.date} onchange={(event) => { store.date = event.currentTarget.value; void store.refresh() }} class="h-9 rounded-md border border-input bg-background px-2 text-xs text-foreground" />
    </label>
    {#if store.date}<button type="button" class="text-xs text-muted-foreground hover:text-foreground" onclick={() => { store.date = ''; void store.refresh() }}>{$_('activityLog.allDates')}</button>{/if}
