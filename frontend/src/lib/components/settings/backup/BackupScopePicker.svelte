<script lang="ts">
  ...
  const allSelected = $derived(accountIds.length > 0 && selected.length === accountIds.length)
...
<div class="relative w-96 max-w-full" bind:this={element}>
  ...
  {#if open}
    <div class="absolute right-0 z-50 mt-1 max-h-64 w-full overflow-y-auto rounded-md border border-border bg-popover p-1 shadow-lg">
      <button type="button" class="flex w-full items-center gap-2 rounded-sm px-2 py-2 text-left text-sm hover:bg-accent" onclick={toggleAll}><Icon icon={allSelected ? 'mdi:checkbox-marked' : 'mdi:checkbox-blank-outline'} class="h-4 w-4 text-primary" />{$_('settingsBackup.scopeAll')}</button>
