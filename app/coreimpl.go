...
// dependencies (mailAPI, composerAPI, contactsAPI, uiRegistry) through the
// v1 interfaces.
//
// One coreImpl is constructed PER extension at App.Startup, scoped to that
// extension's identity via the extensionID field.
//
...
// Contacts returns the host-level contacts surface. Search delegates to the
// local autocomplete store so other extensions can offer contact pickers. Full
// CRUD remains owned by the Contacts extension's Bridge
// (extensions/contacts/backend/bridge.go), and remote-source methods are kept
// as inert compatibility stubs because aulycmail currently has no remote
// contact source implementation.
...
// ListSources returns no sources. Remote contact sources were removed —
// aulycmail keeps a single local address book, so the source list is always
// empty.
...
func (s storageCoreImpl) HostSecrets() coreapi.HostSecrets {
	return hostSecretsCoreImpl{}
}

// hostSecretsCoreImpl is the host implementation of coreapi.HostSecrets.
// Read-only access to credentials whose lifecycle the host owns. No key
// prefixes are currently routed — the surface keeps the coreapi.Storage
// interface satisfied; add prefix routing here if a consumer emerges.
type hostSecretsCoreImpl struct{}

func (h hostSecretsCoreImpl) Get(key string) (string, error) {
	return "", fmt.Errorf("storage.HostSecrets: no host secret for key %q", key)
}
